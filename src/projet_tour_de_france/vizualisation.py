...
MAP_ASPECT_RATIO = 1.4    # pour le rapport y/x-scale 

# on fait une liste contenant les 20 couleurs distinctes de tab20 (cmap="tab20") + 1 couleur supplémentaire
# au total 21 couleurs bien distinctes pour les 21 étapes 
STAGE_COLORS = list(plt.get_cmap("tab20").colors) + ["#4d4d4d"]

# Contour de la France métropolitaine obtenu en reliant les points de
# coordonnées (longitude, latitude), 
# issu d'un fond de carte IGN simplifié. 
# 
# Deux polygones : le continent (FRANCE_MAINLAND) et la Corse (CORSCA). On les stocke
...
FRANCE_OUTLINE = [FRANCE_MAINLAND, CORSICA]   # liste de 2 listes de coordonnées


def _draw_france(ax):
    """Trace les contours de la France (continent + Corse) 
    contour : obtenu en reliant les différents couples de points par une ligne,
    l'intérieur est rempli pour le contraste
    Arguments:
    ax -- matplotlib axe sur lequel on dessine le contour
    """
    for polygon in FRANCE_OUTLINE:                          # on trace successivement les 2 polygones (continent, Corse)
        lons = [point[0] for point in polygon]
        lats = [point[1] for point in polygon]
        ax.fill(lons, lats, color="#eef3ee", zorder=0)    # on remplit la forme délimitée par l'ensemble des points
        ax.plot(lons, lats, color="darkgreen", linewidth=1, alpha=0.4, zorder=1)   #on relie les points
...
def plot_villages(villages, title="Les 120 villages-étapes du Tour 2027"):
    ...
    _draw_france(ax)   #affichage de la carte de France et Corse
    ax.scatter(villages["longitude"], villages["latitude"], s=25, color="darkgreen", zorder=3)
    ax.set_aspect(MAP_ASPECT_RATIO)  # pour corriger l'allure de la carte, définir le rapport d'aspect des axes dans un tracé de Matplotlib. y/x-scale.
    ax.set_title(title)
...
def plot_stages(villages, labels, title, numbered=False):
    ...
    """
    labels = pd.Series(list(labels), index=villages.index)   # labels = série indéxée comme dataframe villages

    fig, ax = plt.subplots(figsize=(8, 8))
    _draw_france(ax)
    #on boucle sur les étiquettes des clusters (les 21 valeurs), et chaque village appartenant au cluster est représenté par un point de même couleur
    for rank, stage in enumerate(sorted(labels.unique())):
        mask = labels == stage    #on crée mask qui contient True ou False sur les labels/villages de même stage
        color = STAGE_COLORS[rank % len(STAGE_COLORS)]   
        ax.scatter(villages.loc[mask, "longitude"], villages.loc[mask, "latitude"],
                   s=30, color=color, zorder=3)    #globalement pour chaque village rattaché au même label, on affecte la même couleur
        if numbered:       # on affiche l'étiquette du cluster à la longitude / latitude moyenne du cluster
...
def plot_stage_zoom(villages, path, stage_number, distance):
    ...
    ax.plot(lons, lats, color="steelblue", linewidth=2, zorder=2)        #on trace la ligne reliant les villages de l'étape
    ax.scatter(lons, lats, s=45, color="steelblue", zorder=3)           #on trace les points représentant les villages de l'étape
    ax.scatter([lons[0]], [lats[0]], s=160, color="gold", edgecolor="black",   #on trace le point de départ couleur or
               zorder=4, label="Départ")
    ax.scatter([lons[-1]], [lats[-1]], s=160, color="black", marker="s",       # on trace le point d'arrivée couleur noir carré
               zorder=4, label="Arrivée")
    for lon, lat, name in zip(lons, lats, names):
        ax.annotate(name, (lon, lat), textcoords="offset points", xytext=(6, 6),  #on ajoute le nom du village à côté du point représentant le village  
                    fontsize=8, zorder=5)
    ax.set_aspect(MAP_ASPECT_RATIO)
...
def print_roadbook(villages, stage_paths, stage_distances):
    ...
    stage_distances -- dict {stage_number: stage length in kilometers}
    """
    total = sum(stage_distances.values())                   #cumul des distances de toutes les étapes
    longest = max(stage_distances, key=stage_distances.get) # numéro d'étape la plus longue (max de la distance)
    shortest = min(stage_distances, key=stage_distances.get)# numéro d'étape la plus courte (min de la distance)   
